...
### 当前版本：0.8.2（2026-08-24）

**更新下载进度与体验修复**：订阅下载进度事件实时显示百分比；未知总大小时进度条以流动动画展示，修复「进度条停滞不动」。

- 更新进度显示：订阅 `DownloadEvent` 累计字节并计算下载百分比写回进度 toast；服务器不带 `Content-Length` 时改为流动动画短条
