...
### 当前版本：0.6.0（2026-08-11）

**PlantUML 图渲染支持**：采用官方纯浏览器方案（TeaVM 编译产物），预览本地渲染 SVG，无需服务器。

- PlantUML 渲染：`plantuml` 代码块动态加载官方 `plantuml.js` + `viz-global.js`（Graphviz 布局）渲染 SVG，样式与 Mermaid 卡片一致；大体积产物（约 5.4MB）走 `public/plantuml/` + 运行时动态 `import` 懒加载，不拖慢启动
- 构建产物：新增 `dist/plantuml/` 静态资源随包分发
