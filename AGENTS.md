...
### 当前版本：0.8.0（2026-08-24）

**中文输入与导航体验**：中文符号自动转 Markdown 记号、快捷键自定义、统一全局搜索条取代跨文件搜索面板、Tab 栏区分工作区内外文件、多语言支持扩展（新增日语）。

- 中文符号转 Markdown 记号：行首输入连续全角符号 + 空格自动转换（`》`→`>`、`···`→```` ` ````、`【】`→`[]`、`·`/`＊`/`－`→列表记号、`＃`→`#`、`～`→`~`、`＊＊＊`→`***`），支持嵌套、保留空格、合并为一个撤销步，代码范围不转换，设置-编辑器可关闭
- 快捷键自定义：设置新增「快捷键」分类，点击徽标录制、恢复默认/禁用、实时冲突检测，保存即生效
