...
### 当前版本：0.8.1（2026-08-24）

**0.8.0 体验优化与修复**：中文符号转 Markdown 记号支持空格/回车双触发、围栏代码块内可输入闭合 ` ``` `；修复 WYSIWYG 引用块行高不足导致内容显示不全。

- 中文符号转 Markdown 记号优化：支持空格与回车两种触发（回车保留自动缩进，仍合并为一个撤销步）；围栏代码块内行首 `···` 视为闭合围栏（CommonMark 语义）允许转换为 ` ``` `，块内其他符号仍不转换
- WYSIWYG 引用块修复：mark decoration 垂直 padding 无法撑开行盒导致多行引用内容挤压/重叠，改用 line-height 提供上下间距、仅保留水平 padding
