...
### 当前版本：0.7.0（2026-08-19）

**用户体验改进**：WYSIWYG 表格对齐与图表实时预览、阅读字体四档预设、非 Markdown 文件支持、文件树宽度调整与折叠等。

- 非 Markdown 文件支持：HTML 沙箱 iframe 预览（源码只读）+ 文本/代码文件源码编辑与语言高亮；无后缀文件 <1MB 按文本文档处理
- 阅读字体四档预设（A 文雅楷体/B 简净无衬线/C 学术衬线/D 技术等宽，默认 D），设置-编辑器切换，预览与 WYSIWYG 统一生效
- 新建文件在有工作区时触发文件树根目录内联命名；文件树支持右缘拖拽调宽 + 收起成细条（状态持久化）；文件树图标按 md/html/其他文本三类区分
- WYSIWYG：光标进入 Mermaid/PlantUML 代码块时块下方实时预览卡片（防抖更新）；修复表格对齐、单行代码等特殊块展示
