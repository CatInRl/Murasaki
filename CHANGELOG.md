...
- 依赖：`vue-i18n` 9.14.4 → 11.1.11（跨 v10/v11）。仓库使用的是 Composition API 模式（`legacy: false`），不受 v10 的 Legacy 重载签名变更影响；已确认未使用 v11 移除的 `tc`/`$tc`，也未使用 `%` 取模插值。注意 v11.4.5+ 要求 Node ≥ 22（首次引入此约束）
