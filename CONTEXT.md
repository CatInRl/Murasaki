...
**"打开文件"无工作区时的行为** —— 打开单个文件作为独立 tab，并**自动以该文件所在目录为工作区**（0.9.0 起，issue #96/#113）：左侧文件树随即显示该目录内容、该文件在树中被选中，同时写入 `lastWorkspacePath` 以便下次启动恢复。目录打开失败仅告警，不影响文件本身打开。用户可通过"打开文件夹"显式切换工作区。
