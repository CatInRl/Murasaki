...
### 引用方式（插入方式设置，issue #151）

设置 → 常规 → **图片插入方式**决定粘贴/拖入外部图片时的**默认**引用方式：

- **本地文件（`imageInsertMode = "file"`，默认）** —— 复制到默认图片目录后插入**相对当前 markdown 文件的路径**，符合 markdown 标准，保证工作区可移植。日常写作推荐这一档。
- **Base64 内嵌（`"base64"`）** —— 直接插入 `data:image/...;base64,...`，单文件便携、不依赖外部资源；代价是 .md 体积膨胀、git diff 噪音大，适合临时分发。
- 两者都不采用绝对路径（跨机器不可用）。

两条补充规则：

- 按住 **`Alt`** 插入时临时用**另一种**方式（本次有效，不改设置）。
- **无工作区**（file 模式无法落盘）或落盘失败时，自动回退为 Base64 内嵌，避免静默丢图。

> `data:image/bmp` / `data:image/svg+xml` 不在 markdown-it 默认放行的 data URI 白名单内（默认只有 gif/png/jpeg/webp），预览侧在 `src/utils/imagePath.ts` 的 `allowInlineImageDataUris` 里显式放行，否则内嵌图片会显示成字面文本。

### 插入场景

1. **粘贴/拖入外部图片** —— 按上面的插入方式处理：
   - file 模式：复制到 `<workspace>/<默认图片目录>/`（默认 `assets/images`，不存在则创建）；
     文件名 `YYYYMMDD-HHmmss-<6位短哈希>.<ext>`，纯 ASCII，无冲突；插入 `![](assets/images/<生成的文件名>)`。
     该目录必须是工作区内的相对路径（Rust 侧拒绝绝对路径与 `..`）。
   - base64 模式：不落盘，插入 `![](data:image/<mime>;base64,...)`。
   - **拖入**时插入点在鼠标**落点**（取不到坐标才回退光标）；拖拽经过编辑区时显示落点提示。
2. **从工作区文件树拖图片到编辑器** —— **不复制、不受插入方式设置影响**，直接写相对路径。
   - 路径为相对当前 markdown 文件的位置（如 `![](../images/a.png)`）。
3. **手动输入 `![](path)`** —— 用户自行确保路径正确，应用不干预。

### 默认图片目录

- 设置项 `defaultImageDir`（默认 `assets/images`），**仅 file 模式生效**。
- 目录不隐藏，用户可见可管理。
- 历史注记：0.9.x 之前该设置项实际未生效（图片固定写入 `assets/`），#151 一并接上。
